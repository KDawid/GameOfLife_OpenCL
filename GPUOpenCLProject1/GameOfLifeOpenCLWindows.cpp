...
void generateInput(cl_int* inputArray, cl_uint arrayWidth, cl_uint arrayHeight)
{
    ...
    {
        inputArray[i] = rand() % 2;
		//printf("Generated number (%i): %i\n", (int) i, (int) inputArray[i]);
    }
}
...
int CreateGameOfLifeBufferArguments(ocl_args_d_t *ocl, cl_int* gameOfLifeInput, cl_int* gameOfLifeOutput, cl_uint arrayWidth, cl_uint arrayHeight)
{
	cl_int err = CL_SUCCESS;
	...
	if (CL_SUCCESS != err)
	{
		LogError("Error: clCreateImage for gameOfLifeInput returned %s\n", TranslateOpenCLError(err));
		return err;
	}
	...
	if (CL_SUCCESS != err)
	{
		LogError("Error: clCreateImage for gameOfLifeOutput returned %s\n", TranslateOpenCLError(err));
		return err;
	}
...
cl_uint SetKernelArguments(ocl_args_d_t *ocl)
{
	...
	if (CL_SUCCESS != err)
	{
		LogError("Error: Failed to set argument GoL Input, returned %s\n", TranslateOpenCLError(err));
		return err;
	}

	err = clSetKernelArg(ocl->kernel, 1, sizeof(cl_mem), (void *)&ocl->golout);
	if (CL_SUCCESS != err)
	{
		LogError("Error: Failed to set argument GoL Output, returned %s\n", TranslateOpenCLError(err));
		return err;
	}
...
cl_uint ExecuteStepOneGenerationKernel(ocl_args_d_t *ocl, cl_uint width, cl_uint height)
{
	cl_int err = CL_SUCCESS;
...
bool ReadGoL(ocl_args_d_t *ocl, cl_uint width, cl_uint height, cl_int *input)
{
	...
	cl_int *resultPtr = (cl_int *)clEnqueueMapImage(ocl->commandQueue, ocl->golout, true, CL_MAP_READ, origin, region, &image_row_pitch, &image_slice_pitch, 0, NULL, NULL, &err);
...
int _tmain(int argc, TCHAR* argv[])
{
	...
	ocl.kernel = clCreateKernel(ocl.program, "StepOneGeneration", &err);
	if (CL_SUCCESS != err)
	{
		LogError("Error: clCreateKernel returned %s\n", TranslateOpenCLError(err));
		return -1;
